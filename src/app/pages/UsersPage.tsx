...
import { Users, Plus, Search, Edit2, Trash2, ShieldAlert, CheckCircle2, X, User, Mail, Phone, Lock } from "lucide-react";

type Role = "resident" | "admin" | "collector";

const initialUsers = [
  { id: 1, name: "Juan Dela Cruz", email: "juan@barangay.ph", phone: "09171234567", role: "resident", address: "Brgy. Lahug, Gorordo Ave.", status: "Active", violations: 3 },
  { id: 2, name: "Maria Santos", email: "maria@barangay.ph", phone: "09181234567", role: "resident", address: "Brgy. Apas, Escario St.", status: "Active", violations: 1 },
  { id: 3, name: "Pedro Reyes", email: "pedro@barangay.ph", phone: "09191234567", role: "resident", address: "Brgy. Capitol Site, Fuente Circle", status: "Active", violations: 2 },
  { id: 4, name: "Ramon Dela Cruz", email: "ramon@collector.ph", phone: "09201234567", role: "collector", address: "Collector - Lahug Area", status: "Active", violations: 0 },
  { id: 5, name: "Pedro Santos", email: "pedro.s@collector.ph", phone: "09211234567", role: "collector", address: "Collector - Apas Area", status: "Active", violations: 0 },
  { id: 6, name: "Ana Garcia", email: "ana@barangay.ph", phone: "09221234567", role: "resident", address: "Brgy. Kamputhaw, Mango Ave.", status: "Suspended", violations: 4 },
  { id: 7, name: "Roberto Cruz", email: "roberto@barangay.ph", phone: "09231234567", role: "resident", address: "Brgy. Mabolo, Salinas Dr.", status: "Active", violations: 1 },
  { id: 8, name: "Liza Mendoza", email: "liza@admin.ph", phone: "09241234567", role: "admin", address: "Barangay Hall - City", status: "Active", violations: 0 },
];
...
export default function UsersPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<typeof initialUsers[0] | null>(null);
  const [form, setForm] = useState({ name: "", email: "", phone: "", role: "resident", address: "" });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editUser) {
      ...
      setUsers([...users, { id: users.length + 1, ...form, status: "Active", violations: 0 }]);
    }
    setShowForm(false);
    setEditUser(null);
    setForm({ name: "", email: "", phone: "", role: "resident", address: "" });
  };

  const openEdit = (user: typeof initialUsers[0]) => {
    setEditUser(user);
    setForm({ name: user.name, email: user.email, phone: user.phone, role: user.role, address: user.address });
    setShowForm(true);
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {role === "admin" && (
          <button
            onClick={() => { setEditUser(null); setForm({ name: "", email: "", phone: "", role: "resident", address: "" }); setShowForm(true); }}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              {[
                { label: "Full Name", key: "name", icon: User, placeholder: "Juan Dela Cruz" },
                { label: "Email", key: "email", icon: Mail, placeholder: "juan@example.com" },
                { label: "Phone", key: "phone", icon: Phone, placeholder: "09171234567" },
                { label: "Address", key: "address", icon: null, placeholder: "Block 1, Zone A" },
              ].map((field) => (
                <div key={field.key}>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="flex-1 py-3 rounded-xl bg-[#2E7D32] text-white hover:bg-[#1B5E20] transition cursor-pointer">
